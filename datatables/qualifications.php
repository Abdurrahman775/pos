...
$sql = "SELECT COUNT(id) FROM qualifications";
$query = $dbh->prepare($sql);
$query->execute();
$totalData = $query->fetchColumn();
$totalFiltered = $totalData;

$sql = "SELECT id, qualification, del_status FROM qualifications";

if(!empty($requestData['search']['value'])) {
    // when there is a search parameter then we have to modify total number filtered rows as per search result.
    $sql.= " WHERE (qualification LIKE '%".$requestData['search']['value']."%' )";
...
while($result = $query->fetch(PDO::FETCH_ASSOC)) {
    ...
    $nestedData = array();
	$nestedData[] = $i;
    $nestedData[] = $result['qualification'];
    ...
    $nestedData[] = "<div class=\"dropdown d-inline-block\">
                        ...
                        <div class=\"dropdown-menu dropdown-menu-right\">
                            <a class=\"dropdown-item\" href=\"edit_qualification.php?" . http_build_query(array("token"=>base64_encode($result['id']))) . "\">Edit</a>
                            <a class=\"dropdown-item\" href=\"javascript: void(0);\" onClick=\"toggle_qualification_status(".$result['id'].")\">" . $del_status . "</a>
                        </div>
                    </div>";
    $data[] = $nestedData;
    $i++;
