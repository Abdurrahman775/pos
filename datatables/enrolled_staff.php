...
$sql = "SELECT COUNT(id) FROM pre_enrolment";
$query = $dbh->prepare($sql);
$query->execute();
$totalData = $query->fetchColumn();
$totalFiltered = $totalData;

$sql = "SELECT id, staff_id, sname, fname, mname, dob, is_linked, reg_date, del_status FROM pre_enrolment";

if(!empty($requestData['search']['value'])) {
    // when there is a search parameter then we have to modify total number filtered rows as per search result.
    $sql.= " AND (staff_id LIKE '%".$requestData['search']['value']."%' ";
...
while($result = $query->fetch(PDO::FETCH_ASSOC)) {
    $enrolment_status = ($result['is_linked'] == 0) ? "<span class='badge badge-danger'>Not Enrolled</span>" : "<span class='badge badge-success'>Enrolled</span>";
    $enrolment_date = !empty($result['reg_date']) ? $result['reg_date'] : "N/A";
    ...
    $nestedData[] = $enrolment_status;
    $nestedData[] = $enrolment_date;
    ...
    $nestedData[] = "<div class=\"dropdown d-inline-block\">
                        ...
                        <div class=\"dropdown-menu dropdown-menu-right\">
                            <a class=\"dropdown-item\" href=\"edit_enrolment.php?" . http_build_query(array("token"=>base64_encode($result['id']))) . "\">Edit</a>
                            <a class=\"dropdown-item\" href=\"javascript: void(0);\" onClick=\"toggle_enrolment_status(".$result['id'].")\">". $del_status ."</a>
                        </div>
                    </div>";
    $data[] = $nestedData;
    $i++;
