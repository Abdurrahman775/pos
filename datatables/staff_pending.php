...
$sql = "SELECT COUNT(id) FROM staff WHERE is_approved= 0 AND del_status= 0";
$query = $dbh->prepare($sql);
$query->execute();
$totalData = $query->fetchColumn();
$totalFiltered = $totalData;

$sql = "SELECT s.id, s.sname, s.fname, s.mname, s.dob, s.mobile1, s.staff_id, s.del_status, sx.sex, r.rank FROM staff s JOIN sexes sx ON s.sex_id = sx.id JOIN ranks r ON s.rank_id = r.id WHERE s.is_approved= 0 AND s.del_status= 0";
...
while($result = $query->fetch(PDO::FETCH_ASSOC)) {
    ...
    $nestedData[] = "<div class=\"dropdown d-inline-block\">
                        ...
                        <div class=\"dropdown-menu dropdown-menu-right\">
                            ...
                            <a class=\"dropdown-item\" href=\"javascript: void(0);\" onClick=\"approve_staff(".$result['id'].")\">Approve</a>
